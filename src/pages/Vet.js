...
export default function Vet() {
  ...
  useEffect(() => {
    ...
    api.get(`/activities/pet/${selectedPet.id}`).then(r => {
      setActivities(r.data.filter(a => a.type === 'VET'));
  ...
  const handleAdd = (e) => {
    ...
    api.post(`/activities/pet/${selectedPet.id}`, { type: 'VET', description, scheduledTime, points: 10 })
      .then(r => {
        setActivities(prev => [...prev, r.data]);
        setDescription(''); setScheduledTime('');
        setSuccess('Consulta cadastrada!');
  ...
  return (
    <div>
      <div style={{display:'flex', alignItems:'center', gap:'8px', marginBottom:'16px'}}>
        ...
        <h2 style={{fontFamily:"'Fredoka One', cursive", fontSize:'1.3rem', color:'#3D2B1A', margin:0}}>Veterinário</h2>
      </div>
      ...
      <div style={{background:'white', borderRadius:'16px', padding:'16px', marginBottom:'16px', boxShadow:'0 2px 10px rgba(0,0,0,0.06)'}}>
        <div style={{fontWeight:700, fontSize:'0.85rem', color:'#6B4226', marginBottom:'12px'}}>Nova consulta</div>
        <form onSubmit={handleAdd}>
          <div style={{display:'grid', gap:'10px'}}>
            <input placeholder="Descrição (ex: Check-up Dr. Ricardo)" value={description} onChange={e => setDescription(e.target.value)}
      ...
      {activities.length === 0 ? (
        <div style={{background:'white', borderRadius:'16px', padding:'24px', textAlign:'center', boxShadow:'0 2px 10px rgba(0,0,0,0.06)'}}>
          ...
          <div style={{fontSize:'0.85rem', color:'#9A7050', fontWeight:600}}>Nenhuma consulta agendada.</div>
        </div>
      ) : (
        <div style={{display:'flex', flexDirection:'column', gap:'8px'}}>
          {activities.map(a => (
            <div key={a.id} style={{background:'white', borderRadius:'14px', padding:'12px 14px', display:'flex', gap:'12px', boxShadow:'0 2px 10px rgba(0,0,0,0.06)', opacity: a.completed ? 0.6 : 1}}>
              <div style={{width:'38px', height:'38px', borderRadius:'10px', background:'#FFF1F2', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.2rem', flexShrink:0}}>🏥</div>
              <div style={{flex:1}}>
