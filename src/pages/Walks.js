...
export default function Walks() {
  ...
  useEffect(() => {
    ...
    api.get(`/activities/pet/${selectedPet.id}`).then(r => {
      setActivities(r.data.filter(a => a.type === 'WALK'));
  ...
  const handleAdd = (e) => {
    ...
    api.post(`/activities/pet/${selectedPet.id}`, { type: 'WALK', description, scheduledTime, points: 20 })
      .then(r => {
        setActivities(prev => [...prev, r.data]);
        setDescription(''); setScheduledTime('');
        setSuccess('Passeio cadastrado!');
  ...
  return (
    <div>
      <div style={{display:'flex', alignItems:'center', gap:'8px', marginBottom:'16px'}}>
        ...
        <h2 style={{fontFamily:"'Fredoka One', cursive", fontSize:'1.3rem', color:'#3D2B1A', margin:0}}>Passeios</h2>
      </div>
      ...
      <div style={{background:'white', borderRadius:'16px', padding:'16px', marginBottom:'16px', boxShadow:'0 2px 10px rgba(0,0,0,0.06)'}}>
        <div style={{fontWeight:700, fontSize:'0.85rem', color:'#6B4226', marginBottom:'12px'}}>Novo passeio</div>
        <form onSubmit={handleAdd}>
          <div style={{display:'grid', gap:'10px'}}>
            <input placeholder="Descrição (ex: Passeio parque)" value={description} onChange={e => setDescription(e.target.value)}
      ...
      {activities.length === 0 ? (
        <div style={{background:'white', borderRadius:'16px', padding:'24px', textAlign:'center', boxShadow:'0 2px 10px rgba(0,0,0,0.06)'}}>
          ...
          <div style={{fontSize:'0.85rem', color:'#9A7050', fontWeight:600}}>Nenhum passeio cadastrado.</div>
        </div>
      ) : (
